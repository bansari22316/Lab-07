...
def print_name_and_age(old_people_list):
    ...
    for person in old_people_list:
        fake_name, fake_age = person
        print(f'{fake_name} is {fake_age} years old.')
...
def save_name_and_age_to_csv(name_and_age_list, csv_path):
    ...
    pandas_dataframe = pd.DataFrame(name_and_age_list, columns=["Name", "Age"])
    pandas_dataframe.to_csv(csv_path, index=False)
    return
